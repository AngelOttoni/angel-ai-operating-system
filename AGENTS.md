...
When the applicable workflow requires architectural review or Angel's
approval, Bia must stop after self-review and await that stage before advancing.
...
- consult the current task or architectural briefing before modifying AOS
  content;
- state the understood scope before execution;
